...
{
    public class CreateTopUpCommand : IRequest<TopUpVm>
    {
        public TopUpReq Model { get; set; } = new TopUpReq();
    }

    public class CreateTopUpCommandHandler : IRequestHandler<CreateTopUpCommand, TopUpVm>
    {
        ...
        public async Task<TopUpVm> Handle(CreateTopUpCommand request, CancellationToken cancellation)
        {
            ...
            try
            {
                ...
                if (await _context.SaveChangesAsync(cancellation) > 0)
                {
                    ...
                    if( await _context.SaveChangesAsync(cancellation) > 0)
                    {
                        List<MemLedger> ledgers = new List<MemLedger>();
                        ...
                        _context.MemLedgers.AddRange(ledgers);
