...
{
    public class GetMemberReportSearchQuery : IRequest<MemberResList>
    {
        public MemberSearchReq Model { get; set; }
        public string WebPath { get; set; }
    }

    public class GetMemberReportSearchQueryHandler : IRequestHandler<GetMemberReportSearchQuery, MemberResList>
    {
        private readonly IMemDbContext _context;
        private readonly IMapper _mapper;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public GetMemberReportSearchQueryHandler(IMemDbContext context, IMapper mapper, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<MemberResList> Handle(GetMemberReportSearchQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                result.DataCount = Convert.ToInt32(data.Count);               
                result.HasError = false;
                result.DataList = data.Select(s => new MemberRes
                {
                    Id = s.Id,
