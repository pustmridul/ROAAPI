...
{
    public class GetMemberByMemberShipNoQuery : IRequest<RegisterMemberVm>
    {
        public string MemberShipNo { get; set; }
    }


    public class GetMemberByMemberShipNoQueryHandler : IRequestHandler<GetMemberByMemberShipNoQuery, RegisterMemberVm>
    {
        ...
        public async Task<RegisterMemberVm> Handle(GetMemberByMemberShipNoQuery request, CancellationToken cancellationToken)
        {
            ...
            var data = await _context.RegisterMembers
                ...
                .AsNoTracking()
                .FirstOrDefaultAsync(q=>q.MembershipNo==request.MemberShipNo.PadLeft(5,'0') && q.IsActive && q.IsMasterMember ==true, cancellationToken);
            ...
            {
                var currentbalance = await _context.MemLedgers.Where(q => q.PrvCusID == data.PrvCusID).SumAsync(s => s.Amount) ?? 0;
                ...
                result.Data = new RegisterMemberRes
                {
                    ...
                    ImgFileUrl = data.ImgFileUrl == null ? baseUrl + "/Members/test.jpg" : baseUrl + "/" + data?.ImgFileUrl,
