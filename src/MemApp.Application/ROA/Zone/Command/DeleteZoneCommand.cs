...
namespace ResApp.Application.ROA.Zone.Command
{
    ...
    public class DeleteZoneCommand : IRequest<Result>
    {
        public int Id { get; set; }
    }

    public class DeleteZoneCommandHandler : IRequestHandler<DeleteZoneCommand, Result>
    {
        private readonly IMemDbContext _context;
        private readonly ICurrentUserService _currentUserService;
        private readonly IPermissionHandler _permissionHandler;
        public DeleteZoneCommandHandler(IMemDbContext context, ICurrentUserService currentUserService, IPermissionHandler permissionHandler)
        {
            _context = context;
        ...
        public async Task<Result> Handle(DeleteZoneCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!await _permissionHandler.HasRolePermissionAsync(3504))
            {
                ...
                result.Messages.Add("Delete Zone Info Denied");
                return result;
            }

            try
            {
                var data = await _context.ZoneInfos
