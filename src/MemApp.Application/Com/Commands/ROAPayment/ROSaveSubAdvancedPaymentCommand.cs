...
namespace ResApp.Application.Com.Commands.ROAPayment
{
   
    public class ROSaveSubAdvancePaymentCommand : IRequest<ListResult<MemberSubPaymentRes>>
    {
    ...
    public class ROSaveSubAdvancePaymentCommandHandler : IRequestHandler<ROSaveSubAdvancePaymentCommand, ListResult<MemberSubPaymentRes>>
    {
        ...
        public ROSaveSubAdvancePaymentCommandHandler(IMemDbContext context, IPermissionHandler permissionHandler,
        ...
        public async Task<ListResult<MemberSubPaymentRes>> Handle(ROSaveSubAdvancePaymentCommand request, CancellationToken cancellation)
        {
                   ...
                   // var result = new MemberSubFeeListVm();
                    ...
                    foreach (var item in checkedList)
                    {
                        if (item.SubscriptionMonth > uncheckedList.FirstOrDefault()?.SubscriptionMonth)
                        {
                            ...
                            result.Messages?.Add("Please pay previous month payment first!");
                            return result;
                        }
                    }
                    // need to fix
                    ...
                    var dueExist = _context.RoSubscriptionDueTemps
                        .AsNoTracking()
                        .Any(x => x.MemberId == request.Model.MemberId && x.IsPaid == false);
                       ...
                       // if (pay == null)
                        if (!checkExist)
                        {
                            ...
                            var subDue = await _context.RoSubscriptionDueTemps
                                 .AsNoTracking()
                                 .AnyAsync(x => x.MemberId == request.Model.MemberId
                            ...
                            if (!subDue)
                            {
                                ...
                                var paymentDetail = new ROASubscriptionPaymentDetail()
                                {
                                    ...
                                    //  LateFeePer = s.LateFePer,
                                    ActualPaymentDate = DateTime.Now,
                                    PaymentFeesDate = s.SubscriptionMonth, // s.PaymentDate,
                    ...
                    _context.RoSubscriptionDueTemps.AddRange( subcriptions );

                    if (subPays.Count > 0)
                    {
                        if (request.Model.topup is not null)
                        {
                            await _mediator.Send(new CreateRoTopUpCommand() { Model = request.Model.topup, MonthCount=checkedList.Count,
